...
async function capitalApi(method, path, body) {
  ...
  parsed._resHeaders = res.headers; // expose response headers for session token extraction
  return parsed;
}
...
async function authenticate() {
  ...
  // Capital.com returns tokens in response headers (CST and X-SECURITY-TOKEN)
  // Fall back to body fields if headers are missing
  ...
  log(`✅ Session aktiv | CST: ${cst ? cst.slice(0,8)+"…" : "MISSING"} | Token: ${securityToken ? securityToken.slice(0,8)+"…" : "MISSING"}`);
  if (!cst || !securityToken) throw new Error("Session-Tokens fehlen in Antwort – prüfe API Key und Credentials");
}
...
{"decisions":[{"epic":"GOLD","action":"BUY","size":0.5,"confidence":82,"reason":"Kurze deutsche Begründung","stopLoss":1800.00,"takeProfit":1950.00}],"marketSummary":"2-3 Sätze Deutsch","riskLevel":"LOW"}

Rules for stopLoss and takeProfit:
- Always provide BOTH stopLoss and takeProfit as absolute price levels (not percentages)
- For BUY: stopLoss BELOW current price (e.g. -2%), takeProfit ABOVE current price (e.g. +3%)
- For SELL: stopLoss ABOVE current price (e.g. +2%), takeProfit BELOW current price (e.g. -3%)
- Use the current mid price from market data to calculate levels
- Ratio: takeProfit distance should be at least 1.5x the stopLoss distance (good risk/reward)
...
async function executeTrade(decision, positions, balance) {
  ...
  if (existingPos) {
    ...
    const closedValue = closedSize && openLevel ? closedSize * openLevel : null;
    try {
      await capitalApi("DELETE", `/positions/${dealId}`);
      log(`${upl >= 0 ? "✅" : "❌"} CLOSE ${existingDir} ${epic} | P&L: CHF ${fmt(upl)}`);

      const pnlLine  = upl >= 0 ? `✅ Gewinn: +CHF ${fmt(upl)}` : `❌ Verlust: -CHF ${fmt(Math.abs(upl))}`;
      const newSaldo = balance + upl;
      const msg =
`${upl >= 0 ? "🟢" : "🔴"} <b>POSITION GESCHLOSSEN</b>

${inst?.emoji} <b>${inst?.name}</b> (${epic})
📍 Richtung: ${existingDir}
📦 Anteile: ${closedSize}
${closedValue ? `💵 Wert: CHF ${fmt(closedValue)}` : ""}
${pnlLine}

💼 Aktueller Saldo: CHF ${fmt(newSaldo)}
💭 ${reason}`;
      await telegramApi(msg);
  ...
  const mktData    = priceHistory[epic] ?? [];
  const mktPrice   = mktData.length ? mktData[mktData.length - 1] : 0;
  const tradeValue = mktPrice ? tradeSize * mktPrice : null;
  const stopLoss   = decision.stopLoss   ?? null;
  const takeProfit = decision.takeProfit ?? null;

  // Build position body – only add SL/TP if provided by AI
  const posBody = {
    epic,
    direction:      action,
    size:           tradeSize,
  ...
  try {
    ...
    log(`${emoji} ${action} ${epic} | Größe: ${tradeSize} | SL: ${stopLoss} | TP: ${takeProfit} | ${confidence}% | ${reason}`);
...
📍 Richtung: ${action}
📦 Anteile: ${tradeSize}
${tradeValue ? "💵 Wert: ~CHF " + fmt(tradeValue) : ""}
